...
export class RoutingController {
    ...
    @ApiResponse({ status: 200, description: 'Optimal route found.', type: OptimizeRouteResponseDto })
    async optimize(@Param('id') id: string, @Body() optimizeRouteDto: OptimizeRouteDto) {
        const startTime = Date.now();
        const network = await this.networksService.findOne(Number(id));
        if (!network) {
            throw new NotFoundException(`Network with ID ${id} not found`);
        }
        const result = this.routingService.calculatePath(network.edges, optimizeRouteDto);
