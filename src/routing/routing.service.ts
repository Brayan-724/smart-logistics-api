...
export class RoutingService {
            ...
            if (graph.has(currentNode)) {
                ...
                if (neighbors) {
                    for (const neighbor of neighbors) {
                        ...
                        if (!costs.has(neighbor.node) || newCost < (costs.get(neighbor.node) ?? Infinity)) {
                            costs.set(neighbor.node, newCost);
                            previous.set(neighbor.node, currentNode);
                            this.insertIntoQueue(queue, { node: neighbor.node, cost: newCost });
                        }
                    }
                }
            }
     ...
     */
    private insertIntoQueue(queue: { node: string; cost: number }[], item: { node: string; cost: number }) {
        let low = 0;
